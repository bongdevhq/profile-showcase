    ...
    if filename.endswith(".md"):
        with open(os.path.join(profiles_dir, filename), 'r') as f:
            ...
            try:
                ...
                image = lines[2].split(":")[1].strip().replace("'", "")                location = lines[2].strip().split(":")[1].strip().replace("'",")
                bio = lines[3].strip().split(":")[1].strip().replace("'",")
                institution = lines[4].strip().split(":")[1].strip().replace("'",")
                organisation = lines[5].strip().split(":")[1].strip().replace("'",")
